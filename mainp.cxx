...
// ----------------------------------------
	...
	if(recover)
	{
		...
		while(1)
		{
			...
			fprintf(flog, "%sbest *old* %2d %15.7e this %15.7e <- ", showbest, -1, bestgoal, resu[nrun]);
			for(int j=0; j<nparm; j++)  fprintf(flog, "%15.7e ", para[nrun][j]);
			fprintf(flog, ": ");
			for(int j=0; j<ngoals; j++)  fprintf(flog, "%15.7e ", subresu[nrun][j]);
			fprintf(flog, "\n");
			nrun++;
		}
	ende:   fclose(fp);
...
// 			if(pmla->nbuf < ngood)	// if we have lost any particles dont count this run
		...
		goal += square( diff/(fit_nef[i]- fit_ne[i]+1)  );
...
// 		system(main5);
	...
	fprintf(flog, "%sbest %5s %2d %15.7e this %15.7e <- ", showbest, timc+11, resource, bestgoal, goal);
	for(int i=0; i<nparm; i++)  fprintf(flog, "%15.7e ", p[i]);
	fprintf(flog, ": ");
	for(int i=0; i<ngoals; i++)  fprintf(flog, "%15.7e ", fit_this[i]);
	fprintf(flog, "\n");
	fflush(flog);
...
isset:		fprintf(stderr, "display is %d\n", display);
	...
	int maxthreads = of->setMaxThreads(maxcores);
	int nparm=mof->nparm;
