...
// 					printf("word, what %d %s  %d\n",  word, what, sofar); 
					...
					if(st != 1)  {fprintf(stderr, "parameter out of range: %s\n", line); myexit(-1);}
// 					fprintf(flog, "found parameter %d\n", parmno);
					var_used[parmno]=1;
			...
			if( ! strcmp(what, "var") ) 
			{
				...
				if( activ &&  activ != 1 )
				{
					fprintf(stderr, "old style !@var format. run ~/RTX/rtxupdate on input file!\n");
					myexit(-1);
				}

				if(st != 5)  {fprintf(stderr, "error in var statement !!!\n"); myexit(-1);}
				if(parmno < 0 || parmno >= maxFitVariables)  {fprintf(stderr, " parameter out of range in var statement !!!\n"); myexit(-1);}
// 				printf("defined parameter %d\n", parmno);
			...
			if( ! strcmp(what, "goal") ) 
			{
				...
				if(err)
				{
					fprintf(stderr, "old style @goal staement!!!!!\n");
					myexit(-1);
				}
...
// 			printf("line %s\n", line);
			...
			while(sscanf(pl, "%d%s%n", words+numsubs, what, &sofar) == 2)
			{
				...
				char * w= what;
				if(*w == '-')  {w++; minus= -1.;}
				if(*w == 'p')  {w++; minus= phFac;   if(!has_beta) { fprintf(stderr, "beta is not defined for p modifier\n"); myexit(-1); }}
				if(*w == 'q')  {w++; minus= 3.*phFac;if(!has_beta) { fprintf(stderr, "beta is not defined for p modifier\n"); myexit(-1); }}
				if(*w == 'b')  {w++; b= 1; if(!has_cath) { fprintf(stderr, "cath is not defined for p modifier\n"); myexit(-1); }}
				if(*w == 'c')  {w++; clike=-2;}
				if(*w == 's')  {w++; slike=-2;}
				int parmno = -1;
				sscanf(w, "%d", &parmno);
...
// 								cath_size,adds[iv],cath_field,cath_s1,new_field);
					...
					{
						char out[100];
						sprintf(out, " %20.10e ", adds[iv]+oldvalue);
...
// 	fprintf(flog, "execute %s\n", command);
	...
	sprintf(t3nam, "calcdirs/%03d/TAPE3.P3", resource);
