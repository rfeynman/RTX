...
// 	int maxthreads = setMaxThreads(0);
// 	On windows systems you have to chage the zero to the number of cores you have.
...
//
//	!@var <active> <parameter number> <initial step size> <lower bound> <upper bound>  	defines a fit parameter. parameter number starts with zero. 
...
//		<lower bound> <upper bound>  are boundaries for the final values
//
//	!@goal <what> <goal> <weight>			defines the figure of merrit. any number of goals can be used
// 		The goal statement has to be put after the parmela elements where the measurement shall be taken.
...
//
//	!@step		no optimizing, but stepping through all active @var values from minimim to maximum in step size (needs work)
//
...
// 
// 	!@cutl <fraction>
// 		For the evaluation uses only a fraction of the beam. "!@cutl 0.1" means that 5% particles are removed from the head and 5% from the tail
// 	!@cutp <fraction>
...
//	At start-up the program creates a file "aaaRTXmainpRuns" containing the pid number of the program. if this file exists no second instance of the program can be started, so that no
//	data corruption happens. The file is usually rmoved at exit, but may have to be removed manually if the program crashed.
// ===================================================================================================================
//
...
const int activePopulation=100;
...
char msg[200];
int myexit(int err, const char * why)
{
	if(pidXvfb > 0) kill(pidXvfb, SIGTERM);
	unlink(thislock);
	fprintf(stderr, "myexit called %s\n", why);
	fflush(stdout);
	fflush(stderr);
	exit(err);
}
...
public:
	...
	animal(int nparm, int nobj)
	{
		this->nparm = nparm;
		this->nobj = nobj;

		father = mother = uncle = aunt = brother = sister = 0;
		pregnant = pareto_rank = evaluated = 0;
		genes = new double[nparm];
		objectives  = new double[nparm];
	}
	~animal()
	{
		delete [] genes;
	...
	int paretoBetter(animal * an);

};

typedef std::vector<animal*> Pop;
Pop population;

int animal::paretoBetter(animal * an)
{
	// true if "this" is better than "an"
	for(int i = 0; i< nobj; i++)
	{
		if( objectives[i] > an->objectives[i]) return 0;
	}
	return 1;
}

bool pareto_sort(animal * an, animal * am)
{
	return( an-> pareto_rank > an-> pareto_rank) ;
}
...
class M2condor  
{
	...
	public:
		...
		int fit_ne[maxFitGoals];		// first number of element in parmela
		int fit_nef[maxFitGoals];		// last number of element in parmela
		...
		double eval(double *  p, double * obj,  int & nerror, int resource=0);
    		double eval();
		void done();
		int run(int resource);
};
...
M2condor::M2condor(char * inpfilename, int recover, int oneTime) 
{
	...
	{
		perror("pmla_log.txt");
		myexit(-1, "cant open pmla_log.txt");
	}
	...
	{
		perror("pmla_log.txt");
		sprintf(msg, "cant open %s", inpfilename);
		myexit(-1, msg);
	}
	...
	{
		sprintf(msg, "input file name must end in .inp, is %s", inputFileName+ls-4);
		myexit(-1, msg);
	}
	getcwd(cwd, 300);
	...
	for(int i=0; i<maxFitVariables; i++) {var_defined[i]= 0; var_used[i]= 0; fit_val[i] =0.; fit_weight[i] =0.; }
...
// 					printf("word, what %d %s  %d\n",  word, what, sofar); 
					...
					if(*w == 's')  w++;
					int parmno = -1;
					int st=sscanf(w, "%d", &parmno);
					if(st != 1)
					{
						sprintf(msg, "parameter out of range: %s\n", line);
						myexit(-1, msg);
					}
					var_used[parmno]=1;
					if(parmno >= nvar) nvar = parmno + 1;
					continue;
				}
				continue;
			}
			...
			if( ! strcmp(what, "var") ) 
			{
				...
				int st=sscanf(pl, "%d%d%lf%lf%lf", &activ, &parmno, &p_mult, &p_lower, &p_upper);
				if( activ &&  activ != 1 )
				{
					myexit(-1, "old style !@var format. run ~/RTX/rtxupdate on input file!");
				}

				if(st != 5)
				{
					myexit(-1, "error in var statement !!!\n");
				}
				if(parmno < 0 || parmno >= maxFitVariables)
				{
					myexit(-1, " parameter out of range in var statement !!!");
				}
// 				printf("defined parameter %d\n", parmno);
				var_defined[parmno]=1;
				if(parmno >= nvar) nvar = parmno + 1;
				if(activ) fprintf(flog, "parameter   %d %f %f %f\n", parmno, p_mult, p_lower, p_upper);

				var_mult[parmno] = p_mult;
				var_lower[parmno] = p_lower;
				var_upper[parmno] = p_upper;
				var_active[parmno] = activ;
				continue;
			}
			...
			if( ! strcmp(what, "goal") ) 
			{
				sscanf(pl, "%s%lf%lf",  fit_what[ngoals], fit_val+ngoals, fit_weight+ngoals); 
				int test;
				int err = sscanf(fit_what[ngoals], "%d", &test);
				if(err)
				{
					myexit(-1, "old style @goal staement!!!!!");
				}

				fit_ne[ngoals] = neo;
				fit_nef[ngoals] = fit_ne[ngoals];

				fprintf(flog, "condition  %d %s %f %f\n",  fit_ne[ngoals], fit_what[ngoals], fit_val[ngoals], fit_weight[ngoals]); 
				ngoals++;
				continue;
			}

			if( ! strcmp(what, "mgoal") ) 
			{
				int active;
				sscanf(pl, "%d%d%d%s%lf%lf", fit_ne+ngoals, fit_nef+ngoals, &active, fit_what[ngoals], fit_val+ngoals, fit_weight+ngoals); 
				if(active)
				{
					fprintf(flog, "condition %d %d %d %s %f %f\n",  fit_ne[ngoals], fit_nef[ngoals], active, fit_what[ngoals], fit_val[ngoals], fit_weight[ngoals]); 
					ngoals++;
				}
				continue;
			}
			...
			if( ! strcmp(what, "beta") ) 
			{
				...
				fprintf(flog, "beta  e %e gamma %e beta %e freq %e phFac %e\n", ener_b, gamma, beta, freq, phFac);
				has_beta=1;
				continue;
			}

			if( ! strcmp(what, "energy") ) 
			{
				sscanf(pl,"%lf", &coolenergy);
				continue;
			}
			...
			if( ! strcmp(what, "slenv") ) 
			{
				...
				{
					perror(slenvLatt);
					sprintf(msg,"cant open %s for read", slenvLatt);
					myexit(-1, msg);
				}
				fclose(fp);
				char file[300];
				...
				if(!fp)
				{
					...
					{
						perror(file);
						sprintf(msg,"cant open %s for read", file);
						myexit(-1, msg);
					}
				}
				fclose(fp);
			...
			if( ! strcmp(what, "cath") ) 
			{
				...
				fprintf(flog, "cath size %f field %f\n", cath_size, cath_field);
				has_cath=1;
				continue;
			}
...
// 			printf("element %-20s\t%d\n", what, neo);
	...
	if(nvar <= 0 && (!oneTime))
	{
		sprintf(msg, "nvar = %d, no parameters specified or used", nvar);
		myexit(-1, msg);
	}
	...
	if(fkit &  !oneTime) myexit(-1, "parameter error");

// 	for(int i=0; i<nvar; i++)
// 	{
// 		fprintf(flog, "var %d parm2var %d ,var2parm %d\n", i, parm2var[i],var2parm[i]);
// 	}
	for(int i=0; i< ngoals; i++) fprintf(flog, "cond %d %s %f %f\n", fit_ne[i], fit_what[i], fit_val[i], fit_weight[i]);
...
// ----------------------------------------
	...
	if(recover)
	{
		...
		{
			perror("recover.hex");
			sprintf(msg,"cant open %s for read\n", "recover.hex");
			myexit(-1, msg);
		}
		...
		{
			sprintf(msg,"Input file name %s is different from name in recover.hex %s", inputFileName, rfile);
			myexit(-1, msg);
		}
		if(nnparm != nparm || nngoals != ngoals)
		{
			sprintf(msg," parateter mismatch nparm=%d nnparm=%d ngoals=%d nngoals=%d", nparm, nnparm, ngoals, nngoals);
			myexit(-1, msg);
		}
...
//     	strcpy(name,"mainp");
	...
	if(nparm == 0 && !oneTime)
	{
		myexit(-1, "No variables set, exiting\n");
	}
	if(ngoals == 0 && !oneTime)
	{
		myexit(-1, "No goals set, exiting\n");
	}
...
int M2condor::run(int thread_number)
{
	animal * an = new animal(nparm, ngoals);
        pthread_mutex_lock( &rtxMutex );
	int size = population.size();
        pthread_mutex_unlock( &rtxMutex );

	double whatnext = double(random())/double(RAND_MAX);
	if( size < activePopulation)
	{
		for(int i=0; i<nparm; i++)
		{
			int var = parm2var[i];
			an->genes[i] = var_lower[var] + (var_upper[var] - var_lower[var])* double(random())/double(RAND_MAX);
		}
	}
	...
	int error;
        eval(an->genes, an->objectives, error, thread_number);
	if(error) return 1;


        pthread_mutex_lock( &rtxMutex );
	for ( Pop::iterator it = population.begin(); it != population.end(); it++)
	{
		animal * am = *it;
		if(an->paretoBetter(am) ) am->pareto_rank++;
		if(am->paretoBetter(an) ) an->pareto_rank++;
	}
	population.push_back(an);
	std::sort(population.begin(), population.end(), pareto_sort);
...
double M2condor::eval()
{
	...
	double genes[200];
	double obj[200];
	for(int i=0; i<nparm; i++)  genes[i]=0;
	int error;
	double r= eval(genes, obj, error, 0);
...
double M2condor::eval(double *  pnow, double * obj,  int & nerror, int resource)
{
	nerror=0;
	...
	sprintf(strippedInputFileName,"pmla_%s_%03d_%s", p1+1, resource, inputFileName);
	// we need this file name with and without the calcdirs/resource part
...
// 			printf("line %s\n", line);
			...
			double high[maxFitVariables];
			double low[maxFitVariables];
			int clike = -1, slike=-1;
			sscanf(pl, "%d%n", &numlines, &sofar);
			pl += sofar;
			while(sscanf(pl, "%d%s%n", words+numsubs, what, &sofar) == 2)
			{
				...
				char * w= what;
				if(*w == '-')  {w++; minus= -1.;}
				if(*w == 'p')  {w++; minus= phFac;   if(!has_beta) { myexit(-1, "beta is not defined for p modifier");  }}
				if(*w == 'q')  {w++; minus= 3.*phFac;if(!has_beta) { myexit(-1, "beta is not defined for p modifier");  }}
				if(*w == 'b')  {w++; b= 1;           if(!has_cath) { myexit(-1, "cath is not defined for p modifier");  }}
				if(*w == 'c')  {w++; clike=-2;}
				if(*w == 's')  {w++; slike=-2;}
				int parmno = -1;
				sscanf(w, "%d", &parmno);
				if( var_active[parmno] )
				{
					adds[numsubs] = minus*pnow[var2parm[parmno]];
					high[numsubs] = var_upper[parmno];
					low[numsubs] = var_lower[parmno];
					bs[numsubs] = b;
					numsubs++;
					if(clike == -2) clike=parmno;
					if(slike == -2) slike=parmno;
				}
			}
...
// 				printf("nwords %d\n", nwords);
				if(clike >= 0 )
				{
					if( slike < 0)
					{
						myexit(-1,"c-parameter set but no s-parameter\n");
					}
					double oldvalueP, oldvalueA;
					int st=sscanf(word[4],"%lf",&oldvalueP);
					if(st != 1)
					{
						sprintf(msg,"not a number in substitution:\n%s\n",line );
						myexit(-1, msg);
					}
...
// 								cath_size,adds[iv],cath_field,cath_s1,new_field);
					...
					{
						char out[100];
						double newValue = adds[iv]+oldValue;
// 						fprintf(flog, "line %s", line);
// 						fprintf(flog, "iv %d old %e new %e low %e up %e\n", iv,oldValue, newValue, var_lower[iv],  var_upper[iv]);
						if(newValue < low[iv]   || newValue > high[iv])
						{
							fprintf(flog, "parameter %d exceedes boundary\n", i);
							nerror=1;
							return 2.e33;
						}
						sprintf(out, " %20.10e ",newValue);
	...
	while( fgets(line,499,fi))
	{
		if( !strncmp(line, "!@if", 4) )
		{
			...
			if(onOff < 0)
			{
				sprintf(msg, "switch '%s' not found \n", ifFlag1);
				myexit(-1, msg);
			}
			if(!onOff)
			{
...
// 	fprintf(flog, "execute %s\n", command);
	...
	{
	   for(int f = fit_ne[i]; f <= fit_nef[i]; f++)
	   {
		if(f != oldne) 	// do the getElement onbly once for each element
		{
			if(f > pmla->nplots)
			{
				sprintf(msg,"goal at %d > nplots %d\n", f , pmla->nplots);
				fprintf(flog  ,"goal at %d > nplots %d\n", f , pmla->nplots);
				delete pmla;
				myexit(-1, msg);
			}
			pmla->getElement(f);
// 			if(pmla->nbuf < ngood)	// if we have lost any particles dont count this run
			if(pmla->nbuf < ninput)	// if we have lost any particles dont count this run
			{
				fprintf(flog, "particles lost, ngood = %d, should be %d\n", pmla->nbuf, ngood);
				fprintf(flog, "element %d found %d particles, should be  input %d, first %d\n", f, pmla->nbuf, ninput, ngood);
				nerror=1;
				goal = 1e33;
				break;
			}
		...
		else
		{
			...
			if(!fi)
			{
				perror("slenv_result.txt");
				myexit(-1, "cant open slenv_result.txt");
			}
			char word[100]; word[0]=0; int ret=0;
			while(  strcmp(word, "result") &&  ret != EOF  ) ret=fscanf(fi,"%s", word);
			if(ret == EOF)
			{
				myexit(-1, "no result in lattice1.txt");
			}
			fscanf(fi,"%lf",fit_this+i);
			fclose(fi);
		}
		obj[i] = (fit_val[i]-fit_this[i])*fit_weight[i];
		goal += square( obj[i]/(fit_nef[i]- fit_ne[i]+1)  );
		fprintf(flog, "value %s at %d = %20.10e objective %e\n", fit_what[i], fit_ne[i], fit_this[i], obj[i]);
	   }
	}
	delete pmla;
...
// 		system(main5);
	...
	{
		sprintf(command,"mv  -f calcdirs/%03d/OUTPAR.TXT calcdirs/%03d/oldOUTPAR.TXT", resource, resource);
		fprintf(flog, "execute %s\n", command);
		system(command);
	}
	fprintf(flog, "%sbest %5s %2d %18.10e this %18.10e <- ", showbest, timc+11, resource, bestgoal, goal);
	for(int i=0; i<nparm; i++)  fprintf(flog, "%18.10e ", pnow[i]);
	fprintf(flog, ": ");
	for(int i=0; i<ngoals; i++)  fprintf(flog, "%18.10e ", fit_this[i]);
	fprintf(flog, "\n");
	fflush(flog);

	for(int j=0; j<nparm; j++) para[nrun][j] = pnow[j];
	...
	{
		perror("recover.hex");
		myexit(-1,"cant open recover.hex");
	}
...
//  copy mad part to temp file with substitutions
	...
	{
		perror( tmp2);
		sprintf(msg,"cant open %s for write", tmp2);
		myexit(-1, msg);
	}
	...
	while(1)
	{
		...
		if( c == '$')
		{
			...
			if(b)
			{
				...
				double new_field= cath_field*cath_size*cath_size/(cath_s1*cath_s1);
				fprintf(flog, "cath_size=%f add=%f cath_field=%f cath_s1=%f new_field=%f\n", cath_size,add,cath_field,cath_s1,new_field);
...
// Also  stripp the !mad marker and copy into pmla_temp.mad 
	...
	{
		perror( tmpm);
		sprintf(msg,"cant open %s for write", tmpm);
		myexit(-1, msg);
	}
	...
	fprintf(flog, "mv print pmla_temp.madout\n");
...
//  now copy the translated lattice into the output file
	...
	{
		perror( tmp3);
		sprintf(msg,"cant open %s for write", tmp3);
		myexit(-1, msg);
	}
	while(1)
	{
...
public:
        M2condor * of;
        int thread_number;
        thread_data(M2condor *of, int thread_number)
        {
                this->of =of;
...
{
        thread_data * dat = (thread_data * ) ptr;
        while( dat->of->run(dat->thread_number));
        delete dat;
	return 0;
}
...
int setMaxThreads(int max)
{
	int maxThreads;
	if(max > 0) maxThreads = max;
	else
	{
		...
		{
			perror("/proc/cpuinfo");
			fprintf(stderr, "use setMaxThreads(n) with n> 0 \n");
			exit(-1);
		}
		char line[1000];
		int n=0;
		while( fgets(line,1000, fp) ) if(!strncmp(line, "processor", 9) ) n++;
		maxThreads = n + max;	// if max is negative it means to reserve other cores for other purposes
	}
	return maxThreads;
}
...
main(int argc, char ** argv, char ** env)
{
	...
	if(!home)
	{
		myexit(-1, "environment variable HOME not set");
	}
	...
	if(display)
	{
		...
		myexit(-1, "both displays are in use\n");
...
isset:		fprintf(stderr, "display is %d\n", display);
	...
	else
	{
		...
		if( ! ParmelaDir ) 
		{
			myexit(-1, "env variable ParmelaDir not set!\n");
		}
	...
	fprintf(mof->flog, "maxcores set to %d\n", maxcores);
	int maxthreads = setMaxThreads(maxcores);
	fprintf(mof->flog, "available threads %d\n", maxthreads);
	if(oneTime)
	{
		maxthreads =1;
		maxthreads = setMaxThreads(maxthreads);
	}
	...
	// if there are *.T7 files or LANL.INI in the directory, create symbolic links in each calc dir
		...
		char * s = dd->d_name;
		int l = strlen(s);
		if( (!strcmp(s+l-3, ".T7"))  || (!strcmp(s, "LANL.INI")) || (!strcmp(s, "PART_RFQ.DST"))   )
		{
                	fprintf(mof->flog, " link %s\n", s);
	...
	{
		if(maxthreads == 1)	// if we can use only one thread we use no threads at all and let the main program do the work
		{
                	thread_data * dat = new thread_data(mof, 0); 
			thread_function((void*) dat);	// thread_function will delete dat.
		}
		else
		{
			pthread_t * thread = new pthread_t[maxthreads];
        		for(int i=0; i< maxthreads; i++) 
        		{   
                		thread_data * dat = new thread_data(mof, i); 
                		int iret = pthread_create( thread+i, NULL, thread_function, (void*) dat);
				if(iret)
				{
					fprintf(stderr, "could not create thread, err = %d\n", iret);
					exit(-1);
				}
        		}   
